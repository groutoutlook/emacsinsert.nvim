...
local alphanum = "abcdefghijklmnopqrstuvwxyz" .. "ABCDEFGHIJKLMNOPQRSTUVWXYZ" .. "0123456789"

emacsinsert.alphanum = alphanum
emacsinsert.default_word_chars = alphanum
emacsinsert.word_chars = {}
...
local function get_word_chars()
	if command_line_mode() then
		return emacsinsert.default_word_chars -- Hmm, we can probably do better than this.
	end

	return vim.b.emacsinsert_word_chars or emacsinsert.word_chars[vim.bo.filetype] or emacsinsert.default_word_chars
end
...
local function new_cursor(s, char_idx, dir, word_chars)
	...
	while can_advance(char_idx) do
		...
		if
			(is_whitespace(c) and consumed_non_whitespace) or (not is_word_char(c, word_chars) and consumed_word_char)
		then
			break
		end
...
function emacsinsert._run_tests()
	local messages = {}
	local function pl(s)
...
local function kill_to(end_line_no, end_cursor_col)
	...
	yank_to_small_delete_register(
		start_cursor_col,
		(end_line_no == start_line_no and end_cursor_col)
			or (end_line_no > start_line_no and last_cursor_col_on_curr_line())
			or (end_line_no < start_line_no and 0)
	)
